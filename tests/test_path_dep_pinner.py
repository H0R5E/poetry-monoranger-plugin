...
@pytest.mark.skipif(not POETRY_V22, reason="Requires Poetry>=2.2")
@pytest.mark.parametrize("disable_cache", [True, False])
def test_executes_path_dep_pinning_for_build_command_include(mock_event_gen_include, disable_cache: bool):
    mock_event = mock_event_gen_include(BuildCommand, disable_cache=disable_cache)
    mock_command = mock_event.command
    config = MonorangerConfig(enabled=True, monorepo_root="../", version_pinning_rule="==")
    path_dep_pinner = PathDepPinner(config)
